...
// Variables
new g_gameid, g_msg_sync, cvar_minplayers, g_special_id, cvar_damage, g_maxplayers, cvar_damage_knife, cvar_rwd_ap
...
public plugin_init()
{
	...
	register_plugin("[ZP] Class Antidoter","1.0", "[P]erfec[T] [S]cr[@]s[H]")
...
public plugin_precache()
{
	...
	if(!amx_load_setting_string(ZP_CUSTOMIZATION_FILE, "Access Flags", "START MODE ANTIDOTER", user_access, charsmax(user_access))) {
		amx_save_setting_string(ZP_CUSTOMIZATION_FILE, "Access Flags", "START MODE ANTIDOTER", "a")
	...
	if(!amx_load_setting_string(ZP_CUSTOMIZATION_FILE, "Access Flags", "MAKE ANTIDOTER", user_access, charsmax(user_access))) {
		amx_save_setting_string(ZP_CUSTOMIZATION_FILE, "Access Flags", "MAKE ANTIDOTER", "a")
		formatex(user_access, charsmax(user_access), "a")
	}
...
public fw_PrimaryAttack_Post(weapon_entity)
{
	...
	static Float:plrViewAngles[3], Float:VecEnd[3], Float:VecDir[3], Float:PlrOrigin[3], hTrace;
...
public native_make_user_antidoter(id)
	return zp_make_user_special(id, g_special_id, 0);
	
public native_get_antidoter_count()
	return zp_get_special_count(0, g_special_id);
